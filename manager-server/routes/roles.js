...
router.get('/list', async (ctx) => {
  const { roleName } = ctx.request.query.params;
...
router.post('/operate', async (ctx) => {
  const { _id, roleName, remark, action } = ctx.request.body.params
  let res, info
  try {
    if (action == "create") {
    ...
    ctx.body = util.fail(`发生错误${error.stack}`)
  }
})

//权限设置
router.post('/update/permission', async (ctx) => {
  const { _id, permissionList } = ctx.request.body.params;
  try {
