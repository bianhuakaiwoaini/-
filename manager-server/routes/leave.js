...
router.get('/list', async (ctx) => {
  const { type, applyState } = JSON.parse(ctx.request.query.params);
  const { page, skipIndex } = util.pager(ctx.request.query.params)
...
router.post("/operate", async (ctx) => {
  const { _id, action, ...params } = ctx.request.body.params
  let authorization = ctx.request.headers.authorization;
  let { data } = util.decoded(authorization)
  ...
  if (action == 'create') {
    //生成申请单号
    let orderNo = "XJ"
    orderNo += util.formatdDate(new Date(), "yyyyMMdd");
...
router.post("/approve", async (ctx) => {
  const { action, remark, _id } = ctx.request.body.params;
  let authorization = ctx.request.headers.authorization;
  let { data } = util.decoded(authorization);
  let params = {}
  try {
    //1:待审批 2：审批中 3：审批拒绝 4：审批通过 5：报废
    ...
    if (action == "refuse") {
      ...
      if (doc.auditFlows.length == doc.auditLogs.length) {
        ctx.body = util.success('当前申请单已经处理，请勿重复提交')
        return;
